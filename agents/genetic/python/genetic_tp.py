...
def get_all_actions_by_type(all_actions):
    """It returns a dictionary with all possible actions, grouped by ActionType."""
    all_actions_by_type = {}
    ScanNetwork_list=[]
    FindServices_list=[]
    ExploitService_list=[]
...
def generate_valid_actions(state):
    """ This function is from NetSecGame. It returns a list with valid actions given a particular state."""
    valid_actions = set()
    #Network Scans
...
def choose_parents_tournament(population, goal, fitness_func, num_per_tournament=2, parents_should_differ=True):
    ...
    for i in range(2):
        ...
        chosen.append(max(options, key=lambda x:fitness_func(x,env.reset(),goal)[0]))
...
def mutation_operator_by_parameter(individual, all_actions_by_type, mutation_prob):
    """Mutation operator: it changes only the parameters, not the ActionType."""
    new_individual = []
...
def mutation_operator_by_action(individual, all_actions, mutation_prob):
    """Mutation operator: it could change the ActionType and the parameters."""
    new_individual = []
    for i in range(len(individual)):
        if random.random() < mutation_prob:
